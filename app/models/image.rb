class Image < ApplicationRecord
  ...
  def draw_todo_image(twitter_id,name,body)
    ...
    new_image = base_image.combine_options do |i|
      i.resize '550x315'
      i.font 'app/assets/fonts/ヒラギノ角ゴシック W3.ttc'
      i.gravity 'center'
      i.draw "text 100,100 '#{body}'"
    end
    new_image.write("app/assets/images/todos/#{twitter_id}/#{name}")
  end
  ...
  def rounding_user_icon(twitter_id,icon_url)
    ...
    rounded_icon = user_icon.composite(circle_flame) do |i|
  ...
  def compose_icon_to_todo(twitter_id,name,icon_url)
    base_image = MiniMagick::Image.open("app/assets/images/todos/#{twitter_id}/#{name}")
